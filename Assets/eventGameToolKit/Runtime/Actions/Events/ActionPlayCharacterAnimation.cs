...
/// <summary>
/// Plays character animations by setting animator parameters (Bool, Int, Float, or Trigger) via UnityEvents or button presses.
/// Common use: Play emote animations on button press, trigger special moves, or activate animations from other scripts.
/// Note: "Trigger" here refers to the Animator parameter type, not physics collider triggers.
/// </summary>
public class ActionPlayCharacterAnimation : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.LogWarning($"ActionPlayCharacterAnimation on {gameObject.name}: Parameter Name is empty!", this);
        }

        // Validate animator
        if (targetAnimator == null)
        {
            Debug.LogWarning($"ActionPlayCharacterAnimation on {gameObject.name}: Target Animator is not assigned!", this);
        }
    }

    /// <summary>
    /// Plays the animation by setting the animator parameter. Call this from UnityEvents or other scripts.
    /// </summary>
    public void PlayAnimation()
    {
        if (targetAnimator == null)
        {
            Debug.LogWarning($"ActionPlayCharacterAnimation on {gameObject.name}: Cannot trigger - no animator assigned!", this);
            return;
        }

        if (string.IsNullOrEmpty(parameterName))
        {
            Debug.LogWarning($"ActionPlayCharacterAnimation on {gameObject.name}: Cannot trigger - parameter name is empty!", this);
            return;
        }
        ...
        {
            Debug.LogWarning($"ActionPlayCharacterAnimation on {gameObject.name}: Animator does not have parameter '{parameterName}'!", this);
            return;
        }
