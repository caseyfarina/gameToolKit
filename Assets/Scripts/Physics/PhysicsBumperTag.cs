...
/// <summary>
/// Tag-filtered bumper that applies repulsion forces with animated visual feedback and cooldown system.
...
{
    [Header("Bumper Tag")]
    [Tooltip("the object tag that will trigger the bumper")]
    ...
    [Header("Events")]
    /// <summary>
    /// Fires when the bumper is triggered by a collision with an object matching the bumper tag
    /// </summary>
    [SerializeField] private UnityEvent onBumperTriggered;
    /// <summary>
    /// Fires when the bumper's cooldown period completes and it can be triggered again
    /// </summary>
    ...
    private void SetupMaterial()
    {
        ...
        if (renderer != null)
        {
            ...
            if (material.HasProperty(emissionPropertyName))
            {
                ...
                try
                {
                    // Try to get as float first (for shadergraph properties)
                    float floatValue = material.GetFloat(emissionPropertyName);
                    isFloatProperty = true;
                    baseEmissionFloat = floatValue;
                    Debug.Log($"Found float property '{emissionPropertyName}' with base value: {baseEmissionFloat}");
                }
                catch (System.Exception)
                {
                    // If float fails, try as a color property (standard URP)
    ...
    {
        if (!collision.gameObject.CompareTag(bumperTag)) return;
